    ...
    return (
        ...
        <div className='flex justify-between pt-[20px]'>
            ...
            <div className='flex hidden md:block'>
                <button className={`mr-[20px] font-Lora hover:text-maroonRed 2xl:text-2xl ${router.pathname == '/aboutme' ? 'font-bold' : 'font-regular'}`} onClick={() => router.push("/aboutme")} >About Me</button>
                <button className={`m-[20px] font-Lora hover:text-maroonRed 2xl:text-2xl ${router.pathname == '/projects' ? 'font-bold' : 'font-regular'}`} onClick={() => router.push("/projects")} >Projects</button>
                <button className={`m-[20px] font-Lora hover:text-maroonRed 2xl:text-2xl ${router.pathname == '/certifications' ? 'font-bold' : 'font-regular'}`} onClick={() => router.push("/certifications")} >Certifications</button>
                <button className={`ml-[20px] font-Lora hover:text-maroonRed 2xl:text-2xl ${router.pathname == '/resume' ? 'font-bold' : 'font-regular'}`} onClick={() => router.push("/resume")} >Resume</button>
            </div>
            <div className='flex md:hidden'>  
                ...
                <div className={`flex flex-col justify-center h-screen w-screen bg-lightPink fixed top-0 left-0 z-[10] transition-transform overflow-x-hidden duration-500 ${isOpen ? "translate-x-full" : ""}`}>
                    <div className='absolute top-0 right-0 m-16 text-2xl'>
                        <button onClick={() => setIsOpen(!isOpen)}><CloseIcon/></button>
                    </div>
                    <div className='absolute top-0 right-left m-16 '>
